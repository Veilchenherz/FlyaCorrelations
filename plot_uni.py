...
AMINO_ACIDS_PER_LINE = 70

df = pd.read_csv("./atom_data_pivot_ohne_shiftx2.csv")
...
row_list = [int(item[0])+3 for item in row_list]
...
#plots = [[0,69], [70,139], [140,209], [210, 279], [280, 349], [350,364]]
...
for plot in plots:
    ...
    x_ticks = range(3, array[:, lower_border:higher_border].shape[1] + 3, 10)  # Alle 10. Ticks auf der x-Achse
    x_labels = row_list[lower_border:higher_border][::10]  # Wähle die Beschriftungen für jeden 10. Punkt
    ax[plots.index(plot)].set_xticks(x_ticks, labels=x_labels, fontsize=14)
    ax[plots.index(plot)].set_yticks(range(len(column_labels)), labels=column_labels, fontsize=14)
...
#colorbar = fig.colorbar(image, ax=ax, orientation='horizontal', fraction=0.04, pad=0.1)
